...
const Card = ({data,handleClick,index}) => {
  ...
  return (
    <div className='w-60 bg-zinc-100 p-4 rounded-md flex gap-4 pb-8 relative mt-20'>
        ...
        <button className={`px-4 py-3 whitespace-nowrap ${added?"bg-teal-700":"bg-orange-600"} absolute bottom-0 left-1/2 -translate-x-[50%] translate-y-[50%] text-white text-xs rounded-full`}
        onClick={()=>handleClick(index)}
        >{added?"Added":"Add to Favourites"}</button>
    </div>
  )
}
